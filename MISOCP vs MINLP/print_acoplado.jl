...
subcost_names=["Hidraulic Power" "Thermal power" "Start-up and-Shut down" "Power shedding" "Well Production" "Gas shedding"]
        ...
        for t in NPER
            cost_t_0[1,t]=[if ngh + ngem >= 1 round(sum(planth[gh, :cost] * php_cx[gh, t] for gh in NGH) / 1000000, digits = 3) else 0 end];
            cost_t_0[2,t]=[round(deltaT * sum(plant[g, :cvnc] * pt_cx[g, t] for g in NGEN) / 1000000 + deltaT * sum(plant[g, :Co] * cg_cx[g, t] for g in NGEN) / 1000000, digits = 3)];
            cost_t_0[3,t]=[round(sum(plant[g, :C_start] * y_cx[g, t] for g in NGEN)/1000000 + sum(plant[g, :C_down] * w_cx[g, t] for g in NGEN) / 1000000, digits = 3)];
            cost_t_0[4,t]=[round(price_shed * deltaT * sum(pshed_cx[n, t] for n in NBUS) / 1000000, digits = 3)];
            cost_t_0[5,t]=[round(sum(well[w, :C_sup_D] * su_cx[w, t] for w in NWE) / 1000000, digits = 3)];
            cost_t_0[6,t]=[round(g_price_shed * sum(gshed_cx[gn, t] for gn in NGN) / 1000000, digits = 3)];
        end
...
if folder_end=="Acoplado_eMISOCP"
       ...
       #Apoyo
       ap_results=Array{Any}(nothing,nper+1,3);
       lp_total=Array{Any}(nothing,nper);
       ap_results[1,1]=string("periods");
       lp_total=sum(Matrix(lip_cx), dims=1)
       for t in NPER
               ap_results[1,3]="ConsumSP";
               ap_results[1,2]="Delta LP";
               ap_results[t+1,1]=t;
               ap_results[t+1,3]=sum(plant[g,:Gnode]!=0 ? cg_cx[g,t] : 0 for g in NGEN);
