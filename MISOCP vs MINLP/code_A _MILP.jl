...
println(string("Modelo MINLP"))
println(string("by Pablo Salas"))

#Packages
using JuMP,LinearAlgebra,DataFrames,CSV,Dates,Gurobi,Ipopt,Juniper,KNITRO,Alpine;
...
# NLP optimizer
const knitro = optimizer_with_attributes(KNITRO.Optimizer, 
                                        "numthreads" => thr) 
...
const solverpro = optimizer_with_attributes(Alpine.Optimizer,
                                        "nlp_solver" =>knitro,
                                        "mip_solver" => gurobi,
...
println(string(""))
println(string("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"))
println(string("Iteration Model MINLP"))

start_time=Base.time_ns();

PG = JuMP.Model(juniper);
...
#= @constraint(PG, c01[t in NPER], u[ss(plant,"Unit","RFILO2TG1D2","order"),t]==0);
@constraint(PG, c02[t in NPER], u[ss(plant,"Unit","RFILO2TG2D2","order"),t]==0);
@constraint(PG, c03[t in NPER], u[ss(plant,"Unit","RFILO2TG3D2","order"),t]==0);
@constraint(PG, c04[t in NPER], u[ss(plant,"Unit","SN_TV1R500","order"),t]==0);
@constraint(PG, c05[t in NPER], u[ss(plant,"Unit","SN_TV2R500","order"),t]==0);
@constraint(PG, c06[t in NPER], u[ss(plant,"Unit","SN_TV3R500","order"),t]==0);
@constraint(PG, c07[t in NPER], u[ss(plant,"Unit","SH_CUMMINS","order"),t]==0);
@constraint(PG, c08[t in NPER], u[ss(plant,"Unit","RECKATG1-D2","order"),t]==0);
@constraint(PG, c09[t in NPER], u[ss(plant,"Unit","PTO_BRVOTG3-D2","order"),t]==0);
@constraint(PG, c10[t in NPER], u[ss(plant,"Unit","PTO_BRVOTG1-D2","order"),t]==0);
@constraint(PG, c11[t in NPER], u[ss(plant,"Unit","PTO_BRVOTG4-D2","order"),t]==0);
@constraint(PG, c12[t in NPER], u[ss(plant,"Unit","PTO_BRVOTG2-D2","order"),t]==0);
@constraint(PG, c13[t in NPER], u[ss(plant,"Unit","CTNEPITG43D2","order"),t]==0);
@constraint(PG, c14[t in NPER], u[ss(plant,"Unit","CTNEPITG42D2","order"),t]==0);
@constraint(PG, c15[t in NPER], u[ss(plant,"Unit","CTNEPITG42D2","order"),t]==0);
@constraint(PG, c16[t in NPER], u[ss(plant,"Unit","ILO2_CARB","order"),t]==0);
@constraint(PG, c17[t in NPER], u[ss(plant,"Unit","CHILCA1CC1GAS","order"),t]==1);
@constraint(PG, c18[t in NPER], u[ss(plant,"Unit","CHILCA1CC2GAS","order"),t]==1);
@constraint(PG, c19[t in NPER], u[ss(plant,"Unit","CHILCA1CC3GAS","order"),t]==1);
@constraint(PG, c20[t in NPER], u[ss(plant,"Unit","KALLPACC1GAS","order"),t]==1);
@constraint(PG, c21[t in NPER], u[ss(plant,"Unit","KALLPACC2GAS","order"),t]==1);
@constraint(PG, c22[t in NPER], u[ss(plant,"Unit","KALLPACC3GAS","order"),t]==1);
@constraint(PG, c23[t in NPER], u[ss(plant,"Unit","OLLEROSCCOMBTG1-GAS","order"),t]==1);
@constraint(PG, c24[t in NPER], u[ss(plant,"Unit","LFLORESCCOMBTG1-GAS","order"),t]==1);  =#
...
@constraint(CPG, pbalance[n in NBUS, t in NPER], 
        ...
        == deltaT*sum( (exist(n,mapbranch[br,2],mapbranch[[br],:])==1) ? f[br,t] : 0 for br in NBR)  
           -deltaT*sum( (exist(mapbranch[br,1],n,mapbranch[[br],:])==1) ? f[br,t] : 0 for br in NBR));            
#Condiciones iniciales
#= @constraint(CPG, c01[t in NPER], u[ss(plant,"Unit","RFILO2TG1D2","order"),t]==0);
@constraint(CPG, c02[t in NPER], u[ss(plant,"Unit","RFILO2TG2D2","order"),t]==0);
@constraint(CPG, c03[t in NPER], u[ss(plant,"Unit","RFILO2TG3D2","order"),t]==0);
@constraint(CPG, c04[t in NPER], u[ss(plant,"Unit","SN_TV1R500","order"),t]==0);
@constraint(CPG, c05[t in NPER], u[ss(plant,"Unit","SN_TV2R500","order"),t]==0);
@constraint(CPG, c06[t in NPER], u[ss(plant,"Unit","SN_TV3R500","order"),t]==0);
@constraint(CPG, c07[t in NPER], u[ss(plant,"Unit","SH_CUMMINS","order"),t]==0);
@constraint(CPG, c08[t in NPER], u[ss(plant,"Unit","RECKATG1-D2","order"),t]==0);
@constraint(CPG, c09[t in NPER], u[ss(plant,"Unit","PTO_BRVOTG3-D2","order"),t]==0);
@constraint(CPG, c10[t in NPER], u[ss(plant,"Unit","PTO_BRVOTG1-D2","order"),t]==0);
@constraint(CPG, c11[t in NPER], u[ss(plant,"Unit","PTO_BRVOTG4-D2","order"),t]==0);
@constraint(CPG, c12[t in NPER], u[ss(plant,"Unit","PTO_BRVOTG2-D2","order"),t]==0);
@constraint(CPG, c13[t in NPER], u[ss(plant,"Unit","CTNEPITG43D2","order"),t]==0);
@constraint(CPG, c14[t in NPER], u[ss(plant,"Unit","CTNEPITG42D2","order"),t]==0);
@constraint(CPG, c15[t in NPER], u[ss(plant,"Unit","CTNEPITG42D2","order"),t]==0);
@constraint(CPG, c16[t in NPER], u[ss(plant,"Unit","ILO2_CARB","order"),t]==0);
@constraint(CPG, c17[t in NPER], u[ss(plant,"Unit","CHILCA1CC1GAS","order"),t]==1);
@constraint(CPG, c18[t in NPER], u[ss(plant,"Unit","CHILCA1CC2GAS","order"),t]==1);
@constraint(CPG, c19[t in NPER], u[ss(plant,"Unit","CHILCA1CC3GAS","order"),t]==1);
@constraint(CPG, c20[t in NPER], u[ss(plant,"Unit","KALLPACC1GAS","order"),t]==1);
@constraint(CPG, c21[t in NPER], u[ss(plant,"Unit","KALLPACC2GAS","order"),t]==1);
@constraint(CPG, c22[t in NPER], u[ss(plant,"Unit","KALLPACC3GAS","order"),t]==1);
@constraint(CPG, c23[t in NPER], u[ss(plant,"Unit","OLLEROSCCOMBTG1-GAS","order"),t]==1);
@constraint(CPG, c24[t in NPER], u[ss(plant,"Unit","LFLORESCCOMBTG1-GAS","order"),t]==1);  =#
...
@constraint(CPG, gflow[pp in NPP, t in NPER],
                    (fg_cx[pp,t]+2*fg_cx[pp,t]*(fg[pp,t]-fg_cx[pp,t]))/(pipe[pp,:Cmn]/stageG)^2 ==  
